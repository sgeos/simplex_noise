...
defmodule AB do
  def write(path, width, height, bit_depth, filename, offset) do
    ...
    {:ok, file} = :file.open("#{path}#{filename}.png", [:write])
      ...
      fn y_in ->
        ...
        |> Enum.map(
          fn x_in ->
            ...
            point = {x, y, z}
            options = %{octaves: 3}
            SimplexNoiseImproved.octave_noise(point, options)
            |> ColorMap.index
          end
        )
        |> IO.iodata_to_binary
...
offset = 0.3
AB.write(path, width, height, bit_depth, "a",  offset/2)
AB.write(path, width, height, bit_depth, "b", -offset/2)
